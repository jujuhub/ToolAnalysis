...
bool DigitBuilder::Initialise(std::string configfile, DataModel &data) {
   /////////////////// Useful header ///////////////////////
   if(verbosity) std::cout << "Initializing Tool DigitBuilder" << std::endl;
   ...
   m_variables.Get("mask", mask); // 
   m_variables.Get("LLAPPDMaskFile", fLLAPPDMaskFile); // 
   ...
   // Read LLAPPD mask file, if given
   if (fLLAPPDMaskFile != "none") {
      if (verbosity > 0) {
         std::cout << "Masking option is ON ! \nLoading LAPPD IDs and center y positions from file: " << fLLAPPDMaskFile << std::endl;
//         mask = true; // this works too! but value set in config file
      }
      this->ReadLLAPPDMaskFile();
   } else {
      if (verbosity > 0) std::cout << "Masking option is OFF. Running reco as usual.." << std::endl;
   } 
// ##################################################################################################
...
bool DigitBuilder::Finalise() {
   ...
   if(verbosity > 0) std::cout<<"DigitBuilder exitting" << std::endl;
   return true;
}
...
bool DigitBuilder::BuildMCRecoDigit() {
   ...
   if(fPhotodetectorConfiguration == "All") {
      ...
      std::cout << "Wrong PhotoDetector Configuration! Allowed configurations: PMT_only, LAPPD_only, All" << std::endl;
      return false;
   }
}
      ...
      for(std::pair<unsigned long,std::vector<MCHit>>&& apair : *fMCPMTHits) {
         ...
         if(det->GetDetectorElement() == "Tank") {
            ...
            if(fParametricModel) {
               ...
               for(MCHit& ahit : hits) {
                  if(verbosity > 3) { std::cout << "This HIT'S TIME AND CHARGE: " << ahit.GetTime() <<
               ...
               if (verbosity > 4) { 
                  std::cout << "PMT position (X<Y<Z): " << to_string(pos_reco.X()) << "," 
                     << to_string(pos_reco.Y()) << "," << to_string(pos_reco.Z()) << std::endl;
               ...
               for(MCHit& ahit : hits) {
                  ...
                  if (verbosity > 4) { 
                     std::cout << "PMT position (X<Y<Z): " << to_string(pos_reco.X()) << "," 
                        << to_string(pos_reco.Y()) << "," << to_string(pos_reco.Z()) << std::endl;
                     std::cout << "PMT Charge,Time: " << to_string(calQ) << "," << to_string(calT) << std::endl;
                  }
                  digitType = RecoDigit::PMT8inch;
                  RecoDigit recoDigit(region, pos_reco, calT, calQ, digitType, PMTId);
                  //recoDigit.Print();
...
bool DigitBuilder::BuildMCLAPPDRecoDigit() {
   ...
   if(fMCLAPPDHits) {
      ...
      for(std::pair<unsigned long,std::vector<MCLAPPDHit>>&& apair : *fMCLAPPDHits) {
         ...
         if(verbosity > 2) std::cout << "Loading in digits for LAPPDID " << LAPPDId << std::endl;

         if(det->GetDetectorElement() == "LAPPD") { // redundant, MCLAPPDHits are LAPPD hits (if statement above)
            ...
            for(MCLAPPDHit& ahit : hits) {
               ...
               if (verbosity > 4) {
                  std::cout << "LAPPD position (X<Y<Z): " << to_string(pos_reco.X()) << "," 
                     << to_string(pos_reco.Y()) << "," << to_string(pos_reco.Z()) << std::endl;
               ...
               if (mask) { 
                  //if(pos_reco.Y() < -20.) continue; // need LAPPDID input and center position

                  // check center positions
//                  std::vector<double> tempLAPPDcenter;
                  for (auto tempVec : lappdcentermap) {
                     std::sort(tempVec.second.begin(), tempVec.second.end());
                     //std::cout << "tempVec.second = { " << tempVec.second.at(0) << ", " << tempVec.second.at(1) << " } " << std::endl;
                     if (tempVec.second.at(0) < tempVec.second.at(1) + 10. && tempVec.second.at(0) > tempVec.second.at(1) - 10.) { // hard-coded rn
                        std::cout << "Error! LAPPDs overlap in mask file. Please fix before continuing! " << std::endl;
                        return false;
                     }
                  }

                  std::vector<double> currLAPPDcenter;
                  for (auto const& pair : lappdcentermap) {
                     //if (LAPPDId != pair.first) continue;
                     currLAPPDcenter = pair.second; // for each long LAPPD ID in mask file
                     for (size_t k = 0; k < currLAPPDcenter.size(); k++) {
                        if (pair.first == LAPPDId && pos_reco.Y() < currLAPPDcenter.at(k) + 10. && pos_reco.Y() > currLAPPDcenter.at(k) - 10.) { // if LAPPDId matches key and falls within range 
                           hitInRange = true;
                        }
                     }
                  }
               } else hitInRange = true;

               if(hitInRange) fDigitList->push_back(recoDigit);
            }
         }
      } // end loop over MCLAPPDHits
   } else {
      std::cout << "No MCLAPPDHits" << std::endl;
      return false;
   }
   return true;
}
...
void DigitBuilder::ReadLLAPPDMaskFile() {
   std::ifstream fin(fLLAPPDMaskFile);

   if (!fin) {
      Log("Unable to open given LAPPD mask file. Using long LAPPDs", v_error, verbosity);
   }

   int llappdid;
   double ycenter1, ycenter2; // TODO: make this general to any num of args

   while(fin >> llappdid >> ycenter1 >> ycenter2) { // TODO: add an if statement for when key already exists
      lappdcentermap[llappdid].push_back(ycenter1);
      lappdcentermap[llappdid].push_back(ycenter2);
   }
}
