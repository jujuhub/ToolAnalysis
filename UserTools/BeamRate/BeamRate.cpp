...
BeamRate::BeamRate():Tool(),
beam_db_store_(false, BOOST_STORE_MULTIEVENT_FORMAT)
{}



bool BeamRate::Initialise(std::string configfile, DataModel &data){
  ...
  m_data->vars.Get("RunNumber",RunNumber);
  m_data->vars.Get("PathToFile",PathToFile);
...
bool BeamRate::Execute(){

  Log("BeamRate: First_entry: "+std::to_string(first_entry),4,verbosity_);
  if (first_entry){
    this->initialise_beam_db();
    first_entry = false;
  } else {
    //Only execute one entry, otherwise return true
    return true;
  }

  Log("BeamRate: Loop through beam status file",4,verbosity_);
  //Check if there is new trigger data available
  
  double bnb_rate = 0.;
  std::map<uint64_t,double> bnb_rate_timestamp;

  bnb_rate = get_bnb_rate();
  bnb_rate_timestamp = get_bnb_rate_timestamp();

  Log("BeamRate tool: Finished Execute step, got a rate of "+std::to_string(bnb_rate)+" Hz",4,verbosity_);

  m_data->CStore.Set("BNBRate",bnb_rate);
  m_data->CStore.Set("BNBRateTimestamp",bnb_rate_timestamp);

  return true;
}


bool BeamRate::Finalise(){

  return true;
}

bool BeamRate::initialise_beam_db(){

  m_variables.Get("verbosity", verbosity_);

  std::stringstream ss_db_filename;
  ss_db_filename << PathToFile << "/" << RunNumber << "_beamdb";
  std::string db_filename = ss_db_filename.str();

  Log("BeamRate tool: Check that beam database file exists",2,verbosity_);
 ...
 std::ifstream dummy_in_file(db_filename);
  if ( !dummy_in_file.good() ) {
    Log("BeamRate tool: Error: Could not open the beam database file \"" + db_filename
     + '\"', 0, verbosity_);
    return false;
  }
  dummy_in_file.close();

  Log("BeamRate tool: Initialise BoostStore",2,verbosity_);
  beam_db_store_.Initialise( db_filename );
  Log("BeamRate tool: Printing BoostStore",2,verbosity_);
  beam_db_store_.Print(false);

  Log("BeamRate tool: Done initialising .. Get BeamDBIndex",4,verbosity_);
  bool got_index = beam_db_store_.Header->Get("BeamDBIndex", beam_db_index_);

  Log("BeamRate tool: Got the beam db index",4,verbosity_);
  if ( !got_index ) {
    Log("BeamRate tool: Error: Could not find the BeamDBIndex entry in the beam database"
  ...
  if ( !got_start ) {
    Log("BeamRate tool: Error: Could not find the StartMillisecondsSinceEpoch entry in the"
  ...
  if ( !got_end ) {
    Log("BeamRate tool: Error: Could not find the EndMillisecondsSinceEpoch entry in the"
      " beam database BoostStore header stored in the file \"" + db_filename
      + '\"', 0, verbosity_);
    return false;
  }
  ...
  if ( !beam_db_store_.Has("BeamDB") ) {
    Log("BeamRate tool: Error: Missing BeamDB key in the beam database BoostStore stored"
      " in the file \"" + db_filename
      + '\"', 0, verbosity_);
    return false;
  }

  Log("BeamRate tool: Loaded beam database entries for times between "
...
double BeamRate::get_bnb_rate(){

  double temp_bnb_rate=0.;



  return temp_bnb_rate;

}

std::map<uint64_t, double> BeamRate::get_bnb_rate_timestamp(){

  std::map<uint64_t, double> temp_bnb_rate_timestamp;

  

  return temp_bnb_rate_timestamp;

}

BeamStatus BeamRate::get_beam_status(uint64_t ns_since_epoch,
  ...
  if ( !found_pot_entry ) {
    Log("BeamRate tool: WARNING: unable to find a suitable entry for "
  ...
  try {
      ...
      if ( low == measurement_map.cend() ) {
        Log("BeamRate tool: WARNING: IF beam database did not have any information"
    ...
    if ( verbosity_ >= 4 ) {
      Log("BeamRate tool: Beam monitoring measurements", 4, verbosity_);
    ...
    Log("BeamRate tool: ANNIE DAQ ms since epoch = " + std::to_string(ms_since_epoch), 4,
      verbosity_);
    ...
    if ( !beam_status.passed_all_cuts() )
    {
      ...
      Log("BeamRate tool: WARNING: bad beam spill", 1, verbosity_);
    }
    ...
    Log("BeamRate tool: WARNING: problem encountered while querying IF beam"
