...
bool LoadANNIEEvent::Execute() {
 ...
 //     delete m_data->Stores["ANNIEEvent"];
    ...
    std::cout <<"Reading in current file "<<current_file_<<": "<<input_filename<<std::endl;
    successful_read = m_data->Stores["ANNIEEvent"]->Initialise(input_filename);
    if (!successful_read){
      std::cout <<"Error: bad filename! Skipping file"<<std::endl;
        ++current_file_;
